...
/**************************** worker task ************************************/
   if (taskid > MASTER) {
      ...
      for(int i=offset;i<(offset+rows);i++) {
			for(int j=0;j<NCA;j++) ss+=abs(a[i][j]);
			printf("\n");
		}
...
/**************************** master task ************************************/
   if (taskid == MASTER) {
      ...
      for (i=1; i<=numworkers; i++) {
         ...
         MPI_Recv(&c[offset], 1, MPI_INT, source, mtype, MPI_COMM_WORLD, &status);
      }
      ...
      for (i=1; i<NRA; i++)
		...
		printf("Time of Execution= %lf",final-start);
	}
